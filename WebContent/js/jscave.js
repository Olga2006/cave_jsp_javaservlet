 ...
 * 
 * @param {function}
 *            ready Callback to invoke on domready
 */
 ...
 * 
 * @param {HTMLElement}
 *            clickTarget Element being clicked
...
/**
 * Webflow.load - Add a window load handler that will run even if load event has
 * already happened
 * 
 * @param {function}
 *            handler
 ...
 * 
 * @copyright Guillermo Grau Panea 2016 @license MIT
 */
...
// --
// -- Usage: `insert<T>(array: Array<T>, idx: number, val: Array<T>|T):
// Array<T>`
// --
// -- ```js
// -- arr = ['a', 'b', 'c']
...
// -- // true
// --
// -- // ... unknown paths create intermediate keys. Numeric segments are
...
// -- Returns a new object built as follows: the overlapping keys from the
// -- second one overwrite the corresponding entries from the first one.
// -- If both the first and second entries are objects they are merged
// recursively.
...
var _ix2Shared = __webpack_require__(3);
 ...
 * https://github.com/bkwld/tram MIT License
 */
 ...
 * minification. If the function has been minified and NODE_ENV !==
...
/**
 * These are private action types reserved by Redux. For any unknown actions,
 * you must return the current state. If the current state is undefined, you
 * must return the initial state. Do not reference these action types directly
 * in your code.
 */
var ActionTypes = {
  INIT: '@@redux/INIT'
...
/**
 * Creates a Redux store that holds the state tree. The only way to change the
 * data in the store is to call `dispatch()` on it.
 * 
 ...
 * 
 * @param {Function}
 *            reducer A function that returns the next state tree, given the
 *            current state tree and the action to handle.
 * 
 * @param {any}
 *            [preloadedState] The initial state. You may optionally specify it
 *            to hydrate the state from the server in universal apps, or to
 *            restore a previously serialized user session. If you use
 *            `combineReducers` to produce the root reducer function, this must
 *            be an object with the same shape as `combineReducers` keys.
 * 
 * @param {Function}
 *            enhancer The store enhancer. You may optionally specify it to
 *            enhance the store with third-party capabilities such as
 *            middleware, time travel, persistence, etc. The only store enhancer
...
function createStore(reducer, preloadedState, enhancer) {
  ...
  /**
	 ...
	 * 
	 * @param {Object}
	 *            action A plain object representing “what changed”. It is a
	 *            good idea to keep actions serializable so you can record and
	 *            replay user sessions, or use the time travelling
	 *            `redux-devtools`. An action must have a `type` property which
	 *            may not be `undefined`. It is a good idea to use string
  ...
  /**
	 ...
	 * 
	 * @returns {observable} A minimal observable of state changes. For more
	 *          information, see the observable proposal:
  ...
  function observable() {
    ...
    return _ref = {
      /**
		 ...
		 * 
		 * @param {Object}
		 *            observer Any object that can be used as an observer. The
		 *            observer object should have a `next` method.
		 * @returns {subscription} An object with an `unsubscribe` method that
		 *          can be used to unsubscribe the observable from the store,
		 *          and prevent further emission of values from the observable.
		 */
...
/**
 * Composes single-argument functions from right to left. The rightmost function
 * can take multiple arguments as it provides the signature for the resulting
 * composite function.
 * 
 * @param {...Function}
 *            funcs The functions to compose.
 * @returns {Function} A function obtained by composing the argument functions
 *          from right to left. For example, compose(f, g, h) is identical to
 *          doing (...args) => f(g(h(...args))).
 */
 ...
 * The `_.matches` iteratee shorthand. _.find(users, { 'age': 1, 'active': true
 * }); // => object for 'pebbles' // The `_.matchesProperty` iteratee shorthand.
 * _.find(users, ['active', false]); // => object for 'fred' // The `_.property`
 * iteratee shorthand. _.find(users, 'active'); // => object for 'barney'
 */
 ...
 *            other The other value to compare.
 * @param {boolean}
 *            bitmask The bitmask flags. 1 - Unordered comparison 2 - Partial
 *            comparison
 * @param {Function}
 ...
 * @param {number}
 *            bitmask The bitmask flags. See `baseIsEqual` for more details.
 * @param {Function}
 *            customizer The function to customize comparisons.
 * @param {Function}
 ...
 * `keysFunc` and `symbolsFunc` to get the enumerable property names and symbols
 * of `object`.
 * 
 * @private
 * @param {Object}
...
// Keep throttled events at ~80fps to reduce reflows while maintaining render
// accuracy
...
function bindEventType(_ref11) {
  ...
  var handleEvent = function handleEvent(nativeEvent) {
    ...
    forEachEventTarget(eventTargets, function (element, eventId, eventStateKey) {
      ...
      // Bypass event handler if current media query is not listed in event
		// config
...
/**
 * Injects CSS into the document to fix behavior issues across different
 * devices.
 */
...
function injectBehaviorCSSFixes(events) {
  ...
  for (var eventId in events) {
    ...
    // add a "cursor: pointer" style rule to ensure that CLICK events get fired
	// for IOS devices
...
function renderInitialGroup(_ref14) {
  ...
  if (actionList && actionList.useFirstGroupAsInitialState) {
    ...
    // Bypass initial state render if current media query is not listed in event
	// config
...
function stopActionGroup(_ref16) {
  ...
  (0, _forEach2.default)(ixInstances, function (instance) {
    ...
    // Validate event key if eventStateKey was provided, otherwise default to
	// true
...
function startActionGroup(_ref17) {
  ...
  // Skip initial state group during action list playback, as it should
	// already be applied
...
/**
 * The base implementation of `assignValue` and `assignMergeValue` without value
 * checks.
 * 
 * @private
 * @param {Object}
...
/*
 * ! Webflow._ (aka) Underscore.js 1.6.0 (custom build) _.each _.map _.find
 * _.filter _.any _.contains _.delay _.defer _.throttle (webflow) _.debounce
 * _.keys _.has _.now
 * 
...
$JSON && $export($export.S + $export.F * (!USE_NATIVE || $fails(function () {
  ...
  stringify: function stringify(it) {
    ...
    if (!isObject(replacer) && it === undefined || isSymbol(it)) return; // IE8
																			// returns
																			// string
																			// on
																			// undefined
...
Webflow.define('ix', module.exports = function ($, _) {
  ...
  function run(trigger, $el, opts, replay) {
    ...
    if (!replay) {
      // Find selector within element descendants, siblings, or query whole
		// document
  ...
  function addStep(_tram, step, meta) {
    ...
    // Once the transition has started, we will always use then() to add to the
	// queue.
    if (meta.start) {
    ...
    // When transitions are not present, set values immediately and continue
	// queue.
    if (transitions == null) {
 ...
 * 
 * @param {Object}
 *            reducers An object whose values correspond to different reducer
 *            functions that need to be combined into one. One handy way to
 *            obtain it is to use ES6 `import * as reducers` syntax. The
 *            reducers may never return undefined for any action. Instead, they
 *            should return their initial state if the state passed to them was
 *            undefined, and the current state for any unrecognized action.
 * 
 ...
 * be invoked directly. This is just a convenience method, as you can call
 ...
 *            actionCreators An object whose values are action creator
 *            functions. One handy way to obtain it is to use ES6 `import * as`
 *            syntax. You may also pass a single function.
 * 
 * @param {Function}
 *            dispatch The `dispatch` function available on your Redux store.
 * 
 * @returns {Function|Object} The object mimicking the original object, but with
 *          every action creator wrapped into the `dispatch` call. If you passed
 *          a function as `actionCreators`, the return value will also be a
 *          single function.
 */
...
/**
 * Creates a store enhancer that applies middleware to the dispatch method of
 * the Redux store. This is handy for a variety of tasks, such as expressing
 ...
 *            source The object of property values to match.
 * @param {Array}
 *            matchData The property names, values, and compare flags to match.
 * @param {Function}
 *            [customizer] The function to customize comparisons.
...
/**
 * A specialized version of `_.filter` for arrays without support for iteratee
 * shorthands.
 * 
 * @private
 * @param {Array}
 ...
 * provided to the memoized function is used as the map cache key. The `func` is
 ...
 * @example
 * 
 * var object = { 'a': 1, 'b': 2 }; var other = { 'c': 3, 'd': 4 };
 * 
 * var values = _.memoize(_.values); values(object); // => [1, 2]
 * 
 * values(other); // => [3, 4]
 * 
 * object.a = 2; values(object); // => [1, 2] // Modify the result cache.
 * values.cache.set(object, ['a', 'b']); values(object); // => ['a', 'b'] //
 * Replace `_.memoize.Cache`. _.memoize.Cache = WeakMap;
 */
 ...
 * The `_.matches` iteratee shorthand. _.findIndex(users, { 'user': 'fred',
 * 'active': false }); // => 1 // The `_.matchesProperty` iteratee shorthand.
 * _.findIndex(users, ['active', false]); // => 0 // The `_.property` iteratee
 * shorthand. _.findIndex(users, 'active'); // => 2
 */
...
function getAffectedElements(_ref3) {
  ...
  if (event && useEventTarget) {
    // eventTarget is not defined when this function is called in a clear
	// request, so find
    // all target elements associated with the event data, and return affected
	// elements.
...
/**
 * Checks `value` to determine whether a default value should be returned in its
 * place. The `defaultValue` is returned if `value` is `NaN`, `null`, or
 * `undefined`.
 * 
 * @static
 * @memberOf _
 ...
 * (result[value] || (result[value] = [])).push(key); return result; }, {}); // => {
...
/**
 * A specialized version of `_.reduce` for arrays without support for iteratee
 * shorthands.
 * 
 * @private
 * @param {Array}
 ...
 *            accumulator The initial value.
 * @param {boolean}
 *            initAccum Specify using the first or last element of `collection`
 ...
 * The `_.matches` iteratee shorthand. _.findLastIndex(users, { 'user':
 * 'barney', 'active': true }); // => 0 // The `_.matchesProperty` iteratee
 * shorthand. _.findLastIndex(users, ['active', false]); // => 2 // The
 * `_.property` iteratee shorthand. _.findLastIndex(users, 'active'); // => 0
 */
 ...
 * `iteratee`. The iteratee is invoked with three arguments: (value, key,
 * object).
 * 
 * @static
 * @memberOf _
 ...
 * 'pebbles', 'age': 1 } };
 * 
 * _.mapValues(users, function(o) { return o.age; }); // => { 'fred': 40,
 * 'pebbles': 1 } (iteration order is not guaranteed) // The `_.property`
 * iteratee shorthand. _.mapValues(users, 'age'); // => { 'fred': 40, 'pebbles':
 ...
 * property are iterated like arrays. To avoid this behavior use `_.forIn` or
 * `_.forOwn` for object iteration.
 * 
 * @static
 * @memberOf _
 * @since 0.1.0
 ...
 * @example
 * 
 * _.forEach([1, 2], function(value) { console.log(value); }); // => Logs `1`
 * then `2`.
 * 
 ...
 * with the last arguments provided to the throttled function. Subsequent calls
 * to the throttled function return the result of the last `func` invocation.
 * 
 ...
 * @param {boolean}
 *            [options.leading=true] Specify invoking on the leading edge of the
 *            timeout.
 * @param {boolean}
 ...
 *          jQuery(element).on('click', _.debounce(sendMail, 300, { 'leading':
 *          true, 'trailing': false })); // Ensure `batchLog` is invoked once
 *          after 1 second of debounced calls. var debounced =
 *          _.debounce(batchLog, 250, { 'maxWait': 1000 }); var source = new
...
var ELEMENT_MATCHES = _ix2Shared.IX2BrowserSupport.ELEMENT_MATCHES; /*
																	 * eslint-env
																	 * browser
																	 */
...
var whenScrollDirectionChange = function whenScrollDirectionChange(handler) {
  return function (options) {
    ...
    // percent top since innerHeight may change for mobile devices which also
	// changes the scrollTop value.
...
var scrollIntoOutOfViewOptions = (0, _extends3.default)({}, baseScrollActionGroupOptions, {
  handler: whenElementVisibiltyChange(function (options, state) {
    ...
    // trigger the handler only once if only one of SCROLL_INTO or SCROLL_OUT_OF
	// event types
    // are registered.
...
exports.default = (_exports$default = {}, (0, _defineProperty3.default)(_exports$default, SLIDER_ACTIVE, getComponentActiveOptions()), (0, _defineProperty3.default)(_exports$default, SLIDER_INACTIVE, getComponentInactiveOptions()), (0, _defineProperty3.default)(_exports$default, DROPDOWN_OPEN, getComponentActiveOptions()), (0, _defineProperty3.default)(_exports$default, DROPDOWN_CLOSE, getComponentInactiveOptions()), (0, _defineProperty3.default)(_exports$default, NAVBAR_OPEN, getComponentActiveOptions(false)), (0, _defineProperty3.default)(_exports$default, NAVBAR_CLOSE, getComponentInactiveOptions(false)), (0, _defineProperty3.default)(_exports$default, TAB_ACTIVE, getComponentActiveOptions()), (0, _defineProperty3.default)(_exports$default, TAB_INACTIVE, getComponentInactiveOptions()), (0, _defineProperty3.default)(_exports$default, ECOMMERCE_CART_OPEN, {
  ...
  handler: function handler(_ref8) {
      ...
      default:
        {
          ...
          // Use isOrContainsElement for mouse events since they are fired
			// from the target
          ...
          // Otherwise we'll need to calculate the mouse position from the
    ...
    // cover case where the event is a mouse out, but the value is not quite at
	// 100%
    if (isMouseOut && (value > 1 - MOUSE_OUT_ROUND_THRESHOLD || value < MOUSE_OUT_ROUND_THRESHOLD)) {
      value = Math.round(value);
    }

    // Only update based on element if the mouse is moving over or has just left
	// the element
 ...
 * @param {boolean}
 *            [predicate=isFlattenable] The function invoked per iteration.
 * @param {boolean}
 *            [isStrict] Restrict to values that pass `predicate` checks.
 * @param {Array}
...
/**
 * Creates a function that'll short out and invoke `identity` instead of `func`
 * when it's called `HOT_COUNT` or more times in `HOT_SPAN` milliseconds.
 * 
 * @private
 * @param {Function}
...
/**
 * Creates a `lodash` object which wraps `value` to enable implicit method chain
 * sequences. Methods that operate on and return arrays, collections, and
 ...
 * of intermediate arrays and can greatly reduce the number of iteratee
 * executions. Sections of a chain sequence qualify for shortcut fusion if the
 * section is applied to an array and iteratees accept only one argument. The
 * heuristic for whether a section qualifies for shortcut fusion is subject to
 * change.
 * 
 ...
 * 
 * The wrapper `Array` methods are: `concat`, `join`, `pop`, `push`, `shift`,
 * `sort`, `splice`, and `unshift`
 * 
 * The wrapper `String` methods are: `replace` and `split`
 * 
 * The wrapper methods that support shortcut fusion are: `at`, `compact`,
 * `drop`, `dropRight`, `dropWhile`, `filter`, `find`, `findLast`, `head`,
 * `initial`, `last`, `map`, `reject`, `reverse`, `slice`, `tail`, `take`,
 * `takeRight`, `takeRightWhile`, `takeWhile`, and `toArray`
 * 
 * The chainable wrapper methods are: `after`, `ary`, `assign`, `assignIn`,
 * `assignInWith`, `assignWith`, `at`, `before`, `bind`, `bindAll`, `bindKey`,
 * `castArray`, `chain`, `chunk`, `commit`, `compact`, `concat`, `conforms`,
 * `constant`, `countBy`, `create`, `curry`, `debounce`, `defaults`,
 * `defaultsDeep`, `defer`, `delay`, `difference`, `differenceBy`,
 * `differenceWith`, `drop`, `dropRight`, `dropRightWhile`, `dropWhile`,
 * `extend`, `extendWith`, `fill`, `filter`, `flatMap`, `flatMapDeep`,
 * `flatMapDepth`, `flatten`, `flattenDeep`, `flattenDepth`, `flip`, `flow`,
 ...
 * `propertyOf`, `pull`, `pullAll`, `pullAllBy`, `pullAllWith`, `pullAt`,
 * `push`, `range`, `rangeRight`, `rearg`, `reject`, `remove`, `rest`,
 * `reverse`, `sampleSize`, `set`, `setWith`, `shuffle`, `slice`, `sort`,
 * `sortBy`, `splice`, `spread`, `tail`, `take`, `takeRight`, `takeRightWhile`,
 * `takeWhile`, `tap`, `throttle`, `thru`, `toArray`, `toPairs`, `toPairsIn`,
 * `toPath`, `toPlainObject`, `transform`, `unary`, `union`, `unionBy`,
 * `unionWith`, `uniq`, `uniqBy`, `uniqWith`, `unset`, `unshift`, `unzip`,
 * `unzipWith`, `update`, `updateWith`, `values`, `valuesIn`, `without`, `wrap`,
 ...
 * `cloneDeepWith`, `cloneWith`, `conformsTo`, `deburr`, `defaultTo`, `divide`,
 * `each`, `eachRight`, `endsWith`, `eq`, `escape`, `escapeRegExp`, `every`,
 * `find`, `findIndex`, `findKey`, `findLast`, `findLastIndex`, `findLastKey`,
 * `first`, `floor`, `forEach`, `forEachRight`, `forIn`, `forInRight`, `forOwn`,
 ...
 * `join`, `kebabCase`, `last`, `lastIndexOf`, `lowerCase`, `lowerFirst`, `lt`,
 * `lte`, `max`, `maxBy`, `mean`, `meanBy`, `min`, `minBy`, `multiply`,
 * `noConflict`, `noop`, `now`, `nth`, `pad`, `padEnd`, `padStart`, `parseInt`,
 * `pop`, `random`, `reduce`, `reduceRight`, `repeat`, `result`, `round`,
 * `runInContext`, `sample`, `shift`, `size`, `snakeCase`, `some`,
 ...
 * wrapped.reduce(_.add); // => 6 // Returns a wrapped value. var squares =
 * wrapped.map(square);
 * 
...
Webflow.define('scroll', module.exports = function ($) {
  ...
  function scroll(el, offset) {
    ...
    // If specified, scroll so that the element ends up in the middle of the
	// viewport
...
Webflow.define('touch', module.exports = function ($) {
  ...
  function Touch(el) {
    ...
    function move(evt) {
      ...
      // Allow swipes while pointer is down, but prevent them during text
		// selection
...
Webflow.define('forms', module.exports = function ($, _) {
  ...
  function init() {
    ...
    // Work around same-protocol IE XDR limitation - without this IE9 and below
	// forms won't submit
  ...
  function findFields(form, result) {
    ...
    // The ":input" selector is a jQuery shortcut to select all inputs, selects,
	// textareas
...
Webflow.define('tabs', module.exports = function ($) {
  ...
  function changeTab(data, options) {
    ...
    function intro() {
      ...
      // We cannot remove the whole inline style because it could be
		// dynamically bound
...
{"events":{"e":{"id":"e","eventTypeId":"PAGE_START","action":{"id":"","actionTypeId":"GENERAL_START_ACTION","config":{"delay":0,"easing":"","duration":0,"actionListId":"a
actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"ease","duration":1000,"target":{"id":"5bf300026add2206a70f249f|a2564043-d0dd-fc3f-5d89-3af25c739ce2"},"value":0,"unit":""}},{"id":"a-12-n-3","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"9d98a81f-fd7b-9565-cfa5-dcdf1b74456c"},"value":1,"unit":""}},{"id":"a-12-n-4","actionTypeId":"STYLE_FILTER","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"5bf300026add2206a70f249f|9dfe1ace-3b85-782c-c65c-06aff72de6e8"},"filters":[{"type":"blur","filterId":"5c45","value":0,"unit":"px"}]}},{"id":"a-12-n-5","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|9bd904f7-a40e-8c28-8620-2f91505ff2a7"}}},{"id":"a-12-n-7","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"15e24f80-2097-7852-a818-41394943df11"},"value":0,"unit":""}},{"id":"a-12-n-6","actionTypeId":"GENERAL_DISPLAY","config":{"delay":1000,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|a2564043-d0dd-fc3f-5d89-3af25c739ce2"}}},{"id":"a-12-n-8","actionTypeId":"GENERAL_DISPLAY","config":{"delay":1000,"easing":"","duration":0,"value":"none","target":{"id":"15e24f80-2097-7852-a818-41394943df11"}}}]}],"createdOn":1542891952847,"useFirstGroupAsInitialState":false},"a-24":{"id":"a-24","title":"AppearMenuHower","actionItemGroups":[{"actionItems":[{"id":"a-24-n-5","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"5bf300026add2206a70f249f|a2564043-d0dd-fc3f-5d89-3af25c739ce2"}}},{"id":"a-24-n","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"ease","duration":1000,"target":{"id":"5bf300026add2206a70f249f|a2564043-d0dd-fc3f-5d89-3af25c739ce2"},"value":1,"unit":""}},{"id":"a-24-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"9d98a81f-fd7b-9565-cfa5-dcdf1b74456c"},"value":0.41,"unit":""}},{"id":"a-24-n-3","actionTypeId":"STYLE_FILTER","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"5bf300026add2206a70f249f|9dfe1ace-3b85-782c-c65c-06aff72de6e8"},"filters":[{"type":"blur","filterId":"5c45","value":4,"unit":"px"}]}},{"id":"a-24-n-4","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|9bd904f7-a40e-8c28-8620-2f91505ff2a7"}}},{"id":"a-24-n-6","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"15e24f80-2097-7852-a818-41394943df11"}}},{"id":"a-24-n-7","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":1000,"target":{"id":"15e24f80-2097-7852-a818-41394943df11"},"value":1,"unit":""}}]}],"createdOn":1542891952847,"useFirstGroupAsInitialState":false},"a-4":{"id":"a-4","title":"AnimationPlantBV","continuousParameterGroups":[{"id":"a-4-p","type":"MOUSE_X","parameterLabel":"Mouse X","continuousActionGroups":[{"keyframe":0,"actionItems":[{"id":"a-4-n","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".vetkaimg","selectorGuids":["ecf14b8c-8f0f-fccf-a981-d2952d398ca5"]},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"keyframe":100,"actionItems":[{"id":"a-4-n-2","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".vetkaimg","selectorGuids":["ecf14b8c-8f0f-fccf-a981-d2952d398ca5"]},"yValue":5,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]}]},{"id":"a-4-p-2","type":"MOUSE_Y","parameterLabel":"Mouse Y","continuousActionGroups":[{"keyframe":0,"actionItems":[{"id":"a-4-n-3","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".vetkaimg","selectorGuids":["ecf14b8c-8f0f-fccf-a981-d2952d398ca5"]},"xValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"keyframe":100,"actionItems":[{"id":"a-4-n-4","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".vetkaimg","selectorGuids":["ecf14b8c-8f0f-fccf-a981-d2952d398ca5"]},"xValue":5,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]}]}],"createdOn":1542961874133},"a-5":{"id":"a-5","title":"KeyTurnBV","actionItemGroups":[{"actionItems":[{"id":"a-5-n","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":0,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-2","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":90,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-3","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":180,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-4","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":270,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-5","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":360,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-6","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":270,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-7","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":180,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-8","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":1000,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":90,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]},{"actionItems":[{"id":"a-5-n-9","actionTypeId":"TRANSFORM_ROTATE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".keyimg","selectorGuids":["4a20e598-3e0e-578f-9217-4dd613f7f6ae"]},"xValue":0,"xUnit":"DEG","yUnit":"DEG","zUnit":"DEG"}}]}],"createdOn":1542986951959,"useFirstGroupAsInitialState":true},"a-6":{"id":"a-6","title":"AppearanceConnectionForm","actionItemGroups":[{"actionItems":[{"id":"a-6-n-5","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|115a62b1-1760-6b42-5f30-b8bffc076ae9"}}},{"id":"a-6-n-8","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|5d708baa-b5a7-02e3-2395-e5a527440510"}}},{"id":"a-6-n-11","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|4521a757-7c79-eac1-de3a-59f10404e610"}}},{"id":"a-6-n-15","actionTypeId":"STYLE_SIZE","config":{"delay":0,"easing":"","duration":500,"locked":false,"target":{"id":"5bf300026add2206a70f249f|d8248e61-fab5-8356-e242-7ca95339d3c0"},"widthUnit":"PX","heightUnit":"AUTO"}}]},{"actionItems":[{"id":"a-6-n-12","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|4521a757-7c79-eac1-de3a-59f10404e610"}}}]},{"actionItems":[{"id":"a-6-n-10","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|5d708baa-b5a7-02e3-2395-e5a527440510"}}}]},{"actionItems":[{"id":"a-6-n-6","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|115a62b1-1760-6b42-5f30-b8bffc076ae9"}}}]},{"actionItems":[{"id":"a-6-n-13","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|0286df74-097a-f033-0838-aa37db8e0fab"}}}]},{"actionItems":[{"id":"a-6-n-14","actionTypeId":"STYLE_SIZE","config":{"delay":0,"easing":"swingFrom","duration":1000,"locked":false,"target":{"id":"5bf300026add2206a70f249f|d8248e61-fab5-8356-e242-7ca95339d3c0"},"heightValue":0,"widthUnit":"PX","heightUnit":"PX"}}]}],"createdOn":1542995152654,"useFirstGroupAsInitialState":true},"a-8":{"id":"a-8","title":"AppearanceInscriptionForm","actionItemGroups":[{"actionItems":[{"id":"a-8-n-5","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|5d708baa-b5a7-02e3-2395-e5a527440510"}}},{"id":"a-8-n-9","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|115a62b1-1760-6b42-5f30-b8bffc076ae9"}}},{"id":"a-8-n-12","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|0286df74-097a-f033-0838-aa37db8e0fab"}}},{"id":"a-8-n-15","actionTypeId":"STYLE_SIZE","config":{"delay":0,"easing":"","duration":500,"locked":false,"target":{"id":"5bf300026add2206a70f249f|d8248e61-fab5-8356-e242-7ca95339d3c0"},"widthUnit":"PX","heightUnit":"AUTO"}}]},{"actionItems":[{"id":"a-8-n-13","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|0286df74-097a-f033-0838-aa37db8e0fab"}}}]},{"actionItems":[{"id":"a-8-n-11","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add2206a70f249f|115a62b1-1760-6b42-5f30-b8bffc076ae9"}}}]},{"actionItems":[{"id":"a-8-n-6","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|5d708baa-b5a7-02e3-2395-e5a527440510"}}}]},{"actionItems":[{"id":"a-8-n-14","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add2206a70f249f|4521a757-7c79-eac1-de3a-59f10404e610"}}}]},{"actionItems":[{"id":"a-8-n-16","actionTypeId":"STYLE_SIZE","config":{"delay":0,"easing":"swingFrom","duration":1000,"locked":false,"target":{"id":"5bf300026add2206a70f249f|d8248e61-fab5-8356-e242-7ca95339d3c0"},"heightValue":0,"widthUnit":"PX","heightUnit":"PX"}}]}],"createdOn":1542995152654,"useFirstGroupAsInitialState":true},"a-20":{"id":"a-20","title":"DisappearLoadigCover","actionItemGroups":[{"actionItems":[{"id":"a-20-n","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026
n-7","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".sortirformimg","selectorGuids":["8d409226-143d-38b8-2ff1-f20dafb6277a"]},"xValue":1,"yValue":1,"locked":true}}]},{"actionItems":[{"id":"a-40-n-8","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"selector":".sortirformimg","selectorGuids":["8d409226-143d-38b8-2ff1-f20dafb6277a"]},"xValue":0.9,"yValue":0.9,"locked":true}}]}],"createdOn":1545371057599,"useFirstGroupAsInitialState":true},"a-27":{"id":"a-27","title":"MoovingBouteilleAjout","continuousParameterGroups":[{"id":"a-27-p","type":"MOUSE_X","parameterLabel":"Mouse X","continuousActionGroups":[{"keyframe":0,"actionItems":[{"id":"a-27-n","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":20,"actionItems":[{"id":"a-27-n-3","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]},{"keyframe":40,"actionItems":[{"id":"a-27-n-4","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":60,"actionItems":[{"id":"a-27-n-5","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]},{"keyframe":80,"actionItems":[{"id":"a-27-n-6","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":100,"actionItems":[{"id":"a-27-n-2","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]}]},{"id":"a-27-p-2","type":"MOUSE_Y","parameterLabel":"Mouse Y","continuousActionGroups":[{"keyframe":0,"actionItems":[{"id":"a-27-n-7","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":20,"actionItems":[{"id":"a-27-n-9","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]},{"keyframe":40,"actionItems":[{"id":"a-27-n-10","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":60,"actionItems":[{"id":"a-27-n-11","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]},{"keyframe":80,"actionItems":[{"id":"a-27-n-12","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":1,"yValue":1,"locked":true}}]},{"keyframe":100,"actionItems":[{"id":"a-27-n-8","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|d41c7a61-743f-0ca3-96a3-a4c2cf6a3b16"},"xValue":0.8,"yValue":0.8,"locked":true}}]}]}],"createdOn":1545371241615},"a-50":{"id":"a-50","title":"SideMenuAnimation","actionItemGroups":[{"actionItems":[{"id":"a-50-n","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce44"},"xValue":1,"yValue":1,"locked":true}}]},{"actionItems":[{"id":"a-50-n-2","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce44"},"xValue":0.9,"yValue":0.9,"locked":true}}]},{"actionItems":[{"id":"a-50-n-3","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce44"},"xValue":1,"yValue":1,"locked":true}}]},{"actionItems":[{"id":"a-50-n-4","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce44"},"xValue":0.9,"yValue":0.9,"locked":true}}]}],"createdOn":1545379570526,"useFirstGroupAsInitialState":true},"a-29":{"id":"a-29","title":"ScaleButtonOuvrirCave","actionItemGroups":[{"actionItems":[{"id":"a-29-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"value":0,"unit":""}},{"id":"a-29-n-5","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"xValue":0.5,"yValue":0.5,"locked":true}}]},{"actionItems":[{"id":"a-29-n-6","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"xValue":1,"yValue":1,"locked":true}},{"id":"a-29-n-3","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"value":1,"unit":""}}]},{"actionItems":[{"id":"a-29-n-4","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"value":0,"unit":""}},{"id":"a-29-n-7","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|ceabc198-b9f3-4422-4ec6-c58c90e859ec"},"xValue":0.9,"yValue":0.9,"locked":true}}]}],"createdOn":1545379616457,"useFirstGroupAsInitialState":true},"a-39":{"id":"a-39","title":"ScaleButtonFermerCave","actionItemGroups":[{"actionItems":[{"id":"a-39-n-7","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"value":0,"unit":""}},{"id":"a-39-n-8","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"xValue":0.5,"yValue":0.5,"locked":true}}]},{"actionItems":[{"id":"a-39-n-9","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"xValue":1,"yValue":1,"locked":true}},{"id":"a-39-n-10","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"value":1,"unit":""}}]},{"actionItems":[{"id":"a-39-n-11","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"value":0,"unit":""}},{"id":"a-39-n-13","actionTypeId":"TRANSFORM_SCALE","config":{"delay":0,"easing":"","duration":2000,"target":{"id":"5bf300026add22b08f0f24a6|cb123da8-822a-0fc2-2992-b1984ac37e4e"},"xValue":0.9,"yValue":0.9,"locked":true}}]}],"createdOn":1545379616457,"useFirstGroupAsInitialState":true},"a-55":{"id":"a-55","title":"AppearFormAjoutComment","actionItemGroups":[{"actionItems":[{"id":"a-55-n","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add225c390f24bd|68ac289a-1a20-a1a9-5347-9c5a565e6fbd"}}},{"id":"a-55-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|68ac289a-1a20-a1a9-5347-9c5a565e6fbd"},"value":0,"unit":""}}]},{"actionItems":[{"id":"a-55-n-3","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add225c390f24bd|68ac289a-1a20-a1a9-5347-9c5a565e6fbd"}}},{"id":"a-55-n-4","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|68ac289a-1a20-a1a9-5347-9c5a565e6fbd"},"value":1,"unit":""}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":true},"a-57":{"id":"a-57","title":"AppearFormAddListCourses","actionItemGroups":[{"actionItems":[{"id":"a-57-n","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add225c390f24bd|43951ab4-6833-f64f-cfca-a6a338ea74b9"}}},{"id":"a-57-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|43951ab4-6833-f64f-cfca-a6a338ea74b9"},"value":0,"unit":""}}]},{"actionItems":[{"id":"a-57-n-3","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"flex","target":{"id":"5bf300026add225c390f24bd|43951ab4-6833-f64f-cfca-a6a338ea74b9"}}},{"id":"a-57-n-4","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|43951ab4-6833-f64f-cfca-a6a338ea74b9"},"value":1,"unit":""}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":true},"a-47":{"id":"a-47","title":"AppearButtonAjoutCave","actionItemGroups":[{"actionItems":[{"id":"a-47-n","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|a6207b9e-8d69-5d74-21cf-257a614694e6"},"yValue":-120,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"actionItems":[{"id":"a-47-n-2","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":3000,"target":{"id":"5bf300026add22b08f0f24a6|a6207b9e-8d69-5d74-21cf-257a614694e6"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":true},"a-65":{"id":"a-65","title":"AppearButtonAjoutProd","actionItemGroups":[{"actionItems":[{"id":"a-65-n-3","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5c87c9153f656c8f0ad43c3c|2268c7f2-5bc3-7d72-53f7-e00c170ae7a0"},"yValue":-143,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"actionItems":[{"id":"a-65-n-4","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":3000,"target":{"id":"5c87c9153f656c8f0ad43c3c|2268c7f2-5bc3-7d72-53f7-e00c170ae7a0"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":true},"a-49":{"id":"a-49","title":"AppearSideMenu","actionItemGroups":[{"actionItems":[{"id":"a-49-n","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"},"yValue":74,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-9","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"}}},{"id":"a-49-n-3","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"},"yValue":83,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-10","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"}}},{"id":"a-49-n-5","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"},"yValue":83,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-11","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"}}},{"id":"a-49-n-7","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"},"yValue":83,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-12","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"}}},{"id":"a-49-n-17","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"}}},{"id":"a-49-n-18","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"},"yValue":74,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-22","actionTypeId":"STYLE_BACKGROUND_COLOR","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3b"},"rValue":0,"gValue":0,"bValue":0,"aValue":0}}]},{"actionItems":[{"id":"a-49-n-19","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"}}},{"id":"a-49-n-20","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":2500,"target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-13","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"}}},{"id":"a-49-n-8","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":2000,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-14","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"}}},{"id":"a-49-n-6","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":1500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-15","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"}}},{"id":"a-49-n-4","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":1000,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"},"yValue":1,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-16","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"}}},{"id":"a-49-n-2","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-49-n-21","actionTypeId":"STYLE_BACKGROUND_COLOR","config":{"delay":0,"easing":"","duration":3000,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3b"},"rValue":0,"gValue":0,"bValue":0,"aValue":0.9}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":true},"a-51":{"id":"a-51","title":"DisAppearSideMenu","actionItemGroups":[{"actionItems":[{"id":"a-51-n-16","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"},"yValue":65,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-51-n-19","actionTypeId":"STYLE_BACKGROUND_COLOR","config":{"delay":0,"easing":"","duration":0,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3b"},"rValue":0,"gValue":0,"bValue":0,"aValue":0}},{"id":"a-51-n-13","actionTypeId":"GENERAL_DISPLAY","config":{"delay":100,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"}}},{"id":"a-51-n-14","actionTypeId":"TRANSFORM_MOVE","config":{"delay":400,"easing":"","duration":1000,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce40"},"yValue":83,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-51-n-15","actionTypeId":"GENERAL_DISPLAY","config":{"delay":500,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce42"}}},{"id":"a-51-n-12","actionTypeId":"TRANSFORM_MOVE","config":{"delay":800,"easing":"","duration":1500,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"},"yValue":83,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-51-n-11","actionTypeId":"GENERAL_DISPLAY","config":{"delay":900,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3e"}}},{"id":"a-51-n-10","actionTypeId":"TRANSFORM_MOVE","config":{"delay":1000,"easing":"","duration":2000,"target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"},"yValue":74,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-51-n-9","actionTypeId":"GENERAL_DISPLAY","config":{"delay":1100,"easing":"","duration":0,"value":"none","target":{"id":"3282efeb-b036-c5b8-761f-b8864321ce3c"}}},{"id":"a-51-n-18","actionTypeId":"TRANSFORM_MOVE","config":{"delay":1200,"easing":"","duration":2500,"target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"},"yValue":74,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"id":"a-51-n-17","actionTypeId":"GENERAL_DISPLAY","config":{"delay":1300,"easing":"","duration":0,"value":"none","target":{"id":"518b3de9-ff38-a88a-99d4-8fd16f17c843"}}}]}],"createdOn":1545392317354,"useFirstGroupAsInitialState":false},"a-32":{"id":"a-32","title":"Opendescriptionbouteille","actionItemGroups":[{"actionItems":[{"id":"a-32-n-2","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"}}},{"id":"a-32-n-6","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"},"value":0,"unit":""}}]},{"actionItems":[{"id":"a-32-n-3","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"}}},{"id":"a-32-n-5","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"},"value":1,"unit":""}}]}],"createdOn":1548449758747,"useFirstGroupAsInitialState":true},"a-33":{"id":"a-33","title":"Closedescriptionbouteille","actionItemGroups":[{"actionItems":[{"id":"a-33-n-4","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":0,"target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"},"value":0,"unit":""}}]},{"actionItems":[{"id":"a-33-n-3","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add22b08f0f24a6|e8fd9fe9-a7f2-e57a-19be-6a01c7e6bbe3"}}}]}],"createdOn":1548449758747,"useFirstGroupAsInitialState":false},"a-59":{"id":"a-59","title":"AppearButtonAjoutBouteille","actionItemGroups":[{"actionItems":[{"id":"a-59-n","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add225c390f24bd|6bd8ccb5-cec5-cf51-ad9e-85eceaa51488"},"yValue":-178,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"actionItems":[{"id":"a-59-n-2","actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"swingTo","duration":2000,"target":{"id":"5bf300026add225c390f24bd|6bd8ccb5-cec5-cf51-ad9e-85eceaa51488"},"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]}],"createdOn":1551689824840,"useFirstGroupAsInitialState":false},"a-62":{"id":"a-62","title":"AppearPlusInfoBouteilleDansCave","actionItemGroups":[{"actionItems":[{"id":"a-62-n","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"}}},{"id":"a-62-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"},"value":0,"unit":""}}]},{"actionItems":[{"id":"a-62-n-3","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"block","target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"}}},{"id":"a-62-n-4","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"},"value":1,"unit":""}}]}],"createdOn":1551690076756,"useFirstGroupAsInitialState":true},"a-63":{"id":"a-63","title":"DisAppearPlusInfoBouteilleDansCave","actionItemGroups":[{"actionItems":[{"id":"a-63-n-2","actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"","duration":500,"target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"},"value":0.02,"unit":""}},{"id":"a-63-n","actionTypeId":"GENERAL_DISPLAY","config":{"delay":0,"easing":"","duration":0,"value":"none","target":{"id":"5bf300026add22b08f0f24a6|0ce904f8-f0a8-8245-95cf-d79901a8a71e"}}}]}],"createdOn":1551690076756,"useFirstGroupAsInitialState":false}},"site":{"mediaQueries":[{"key":"main","min":992,"max":10000},{"key":"medium","min":768,"max":991},{"key":"small","min":480,"max":767},{"key":"tiny","min":0,"max":479}]}}
);
